...
- Decide whether --source-kind/--source belong on commands at all. Key insight: when used on `usage add`, it's really just a convenience for updating `sysand.toml` with a source override — the end state is identical to editing config manually. The question is whether that shortcut justifies 8 source kind variants, a SourceSpec type, and flags on multiple commands, given the "edit the file directly" precedent for field-level operations. If removed, the SourceKind enum and SourceSpec type in public-api.md become internal to config parsing only.
