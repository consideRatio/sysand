...
fn template_expands_with_full_percent_encoding() {
    ...
    let url = template.expand("admin/proj0/4.0.20/project.kpar").unwrap();
    assert_eq!(
        url.as_str(),
...
fn template_encodes_non_unreserved_bytes() {
    ...
    let url = template.expand("a b/+%/ö.json").unwrap();
    assert_eq!(
        url.as_str(),
...
fn template_preserves_unreserved_bytes() {
    ...
    let url = template.expand("Az0-9_.~x").unwrap();
...
fn template_placeholder_in_query_expands() {
    ...
    let url = template.expand("a/b.json").unwrap();
    assert_eq!(
        url.as_str(),
...
fn raw_template_keeps_slashes_literal() {
    ...
    let url = template.expand("admin/proj0/versions.json").unwrap();
    assert_eq!(
        url.as_str(),
...
fn raw_template_encodes_within_segments() {
    ...
    let url = template.expand("a b/ö/c+d.json").unwrap();
    assert_eq!(
        url.as_str(),
...
fn resolve_on_root_matches_append_semantics() {
    let location = IndexLocation::parse("https://example.org/index").unwrap();
    let url = location.resolve("admin/proj0/versions.json").unwrap();
    assert_eq!(
        url.as_str(),
...
fn unknown_placeholder_is_rejected() {
    ...
    assert!(matches!(
        &error,
        IndexLocationError::UnknownPlaceholder { placeholder, .. } if placeholder == "{file}"
    ));
}
...
#[test]
fn display_round_trips_the_configured_string() {
    assert_eq!(
