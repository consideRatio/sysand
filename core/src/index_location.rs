...
pub const PATH_RAW_PLACEHOLDER: &str = "{path_raw}";
...
pub enum IndexLocationError {
    ...
    #[error(
        "index URL template `{url}` expands to an invalid URL: {source}; \
         note that `{{path}}` may only appear in the path or query of an \
         absolute HTTP(S) URL"
    )]
    InvalidTemplate {
        url: Box<str>,
    ...
    UnsupportedScheme { url: Box<str>, scheme: String },
...
pub enum ResolveUrlError {
    #[error("cannot construct URL from `{root}` and `{path}`: {source}")]
    Join {
        root: Box<str>,
        path: Box<str>,
        #[source]
    ...
    #[error(
        "expanding index URL template `{template}` with `{path}` produced an invalid URL: {source}"
    )]
    Expand {
        template: Box<str>,
        path: Box<str>,
...
impl IndexUrlTemplate {
    ...
    fn parse(raw: &str) -> Result<Self, IndexLocationError> {
        ...
        // Fragments are silently dropped by servers; in a template they are
        ...
        let expanded = match template.expand("pr obe/probe") {
            ...
            Err(ResolveUrlError::Expand { source, .. } | ResolveUrlError::Join { source, .. }) => {
...
impl IndexLocation {
    ...
    /// or `}` are treated as URL templates (braces are not valid in URLs,
    /// so no plain URL is misclassified); anything else is parsed as a
    /// plain base URL exactly as before.
...
fn validate_expanded_shape(raw: &str, expanded: &url::Url) -> Result<(), IndexLocationError> {
    if !matches!(expanded.scheme(), "http" | "https") {
        return Err(IndexLocationError::UnsupportedScheme {
            url: raw.into(),
            scheme: expanded.scheme().to_owned(),
        });
    }
