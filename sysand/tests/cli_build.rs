...
fn project_build_with_readme() -> Result<(), Box<dyn std::error::Error>> {
    let (_temp_dir, cwd, out) =
        run_sysand(["init", "--version", "1.2.3", "--name", "test_readme"], None)?;
...
fn project_build_without_readme() -> Result<(), Box<dyn std::error::Error>> {
    let (_temp_dir, cwd, out) =
        run_sysand(["init", "--version", "1.2.3", "--name", "test_no_readme"], None)?;
...
fn workspace_build_with_readme() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (project_name, expected_readme) in [("project1", "# Project 1\n"), ("project2", "# Project 2\n")] {
        let kpar_path = cwd
            .join("output")
            .join(format!("{}-1.2.3.kpar", project_name));
        ...
        let mut content = String::new();
        readme.read_to_string(&mut content)?;
        assert_eq!(content, expected_readme, "README mismatch for {project_name}");
    }
