...
`index.json` MUST be served whenever the URL is in fact a sysand index. A
registry with no projects serves `{"projects": []}` with 200 OK. A 404 on
`index.json` signals "this URL is not a sysand index" (typo, wrong
deployment, server down), not "empty index" — the client propagates it as
a hard error so the resolver chain can skip this index and try the next
source rather than silently continue against a misconfigured base URL.
...
The server MUST emit entries newest-first by parsed `semver::Version`. The
client does not re-sort on ingest; `versions_async` streams entries in wire
order, and display-oriented consumers (e.g. `sysand list-versions`) rely on
the server's ordering directly. Pubgrub re-sorts internally when solving,
so correctness does not depend on this ordering — it's a display-only
contract.
